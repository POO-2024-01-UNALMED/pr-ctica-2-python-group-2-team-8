...
class ServicioSouvenirs(Servicio):
    ...
    def descuentar_por_compra(self, metodo):
        ...
        if metodo.get_nombre().lower() != "efectivo":
            for producto in self.orden:
                if (producto.get_tamaño().lower() in ["katana", "emociones"]) and (producto.get_precio() > 120000):
                    self.valor_pedido -= self.valor_pedido * 0.1
                    return True
            return False
        return False

    def actualizar_inventario(self):
        """Description: Este método filtra y actualiza los productos que hay en el inventario dependiendo de la sucursal de cine y del tipo del producto.
           return inventario: Genera un inventario con los productos disponibles del servicio según su localidad para tener una carta más eficiente a la hora de mostrarla al cliente.
        """
        inventario_general = self.cliente.get_cine_actual().get_inventario_cine()
        inventario = []
        for producto in inventario_general:
            if producto.get_tipo_producto().lower() == "souvenir" and len(inventario_general) > 0:
                inventario.append(producto)
        return inventario

    def procesar_pago_realizado(self, cliente):
        """Description: Este método me restablece los métodos de pago del cliente, además de restablecer la orden y el valor del pedido.
    ...
    def factura(self):
        """Description: Me genera una factura la cual muestra toda la orden con su información y fecha de compra.
           return: Genera un String con la fecha actual, el nombre del cliente, y el total menos sus descuentos.
        """
        factura = (
            "                          CINEMAR \n"
            "==================== Factura de Souvenir ====================\n"
